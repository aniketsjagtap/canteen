    ...
    function index()
    {
		if ($this->auth->loggedin()) {
			...
			$this->template
				...
				->build('report/getData',$this->data);
		}
		else{
			$this->template
	...
	function getData()
	{
		if ($this->auth->loggedin()) {
			...
			$this->data['category'] = $this->Category_model->get_all_category();
				...
				$this->form_validation->set_rules('category_id', '<b>Product Category</b>', 'trim|required|integer');
			...
			{         
						...
						for($i=0;$i<sizeof($this->data['sales_report']);$i++)
				{
					...
					$this->data['sales_report'][$i]['rawMaterialType'] = $this->data['rawMaterialType'][$i]['type_id'];
					$this->data['category_data'][$i] = $this->Type_model->get_Type($this->data['sales_report'][$i]['rawMaterialType']);
